...
cols_selected = yaml.safe_load(open("utils/vars_selection.yaml", "r") )
df_all = pd.DataFrame()
...
for rentree in ALL_RENTREES:
    ...
    df_all = (pd.concat([df_all, 
                        ...
                        .groupby(list(set(cols_selected["all_select"]).difference(set(cols_selected["calcs_select"]))), dropna=False)[cols_selected['calcs_select']]
                        .sum()
                        .reset_index()
    ]))
...
df=get_individual_source(zip_path, 'sise', '2011')
